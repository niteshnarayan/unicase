...
public class MECommentReplyWidget extends Composite {
	...
	private final class CreateCommentCommand extends EMFStoreCommand {
		...
		private CreateCommentCommand(EList<OrgUnit> recipientsList,
				Text inputText) {
			super();
		...
		protected void doRun() {
			CompositeOperationHandle compositeOperationHandle = WorkspaceManager
					.getProjectSpace(modelElement).beginCompositeOperation();
			final Comment newComment = RationaleFactory.eINSTANCE
					.createComment();
			...
			try {
				compositeOperationHandle.end("add comment",
						"Added new comment", ModelUtil.getProject(modelElement)
								.getModelElementId(modelElement));
				notifyCommentAdded(newComment);
			} catch (InvalidHandleException e) {
				WorkspaceUtil
						.logException(
								"Could not add comment, there was a composite operation handle exception.",
								e);
			}
		}
	}
	 ...
	 * @param currentUser
	 *            the User which is entering this comment
	 */
	public MECommentReplyWidget(UnicaseModelElement comment,
			Composite composite, ESUsersession currentUser) {
		...
		GridLayoutFactory.fillDefaults().spacing(0, 0)
				.extendedMargins(40, 175, 7, 17).applyTo(inputComposite);
		...
		inputComposite.addPaintListener(new PaintListener() {
			public void paintControl(PaintEvent e) {
				...
				gc.fillRoundRectangle(x + 30, y - 25,
						inputComposite.getSize().x - 200,
				...
				gc.setForeground(getDisplay().getSystemColor(
						SWT.COLOR_DARK_GRAY));
				gc.drawRoundRectangle(x + 30, y - 25,
						inputComposite.getSize().x - 200,
						inputComposite.getSize().y + 15, 20, 20);
			}
		});
	...
	private void createInputEntry() {
		...
		final Text inputText = new Text(inputEntry, SWT.MULTI | SWT.WRAP
				| SWT.BORDER | SWT.V_SCROLL);
		GridDataFactory.fillDefaults().grab(true, false).hint(-1, 40)
				.applyTo(inputText);
		...
		GridDataFactory.fillDefaults().align(SWT.END, SWT.BEGINNING)
				.applyTo(closeButton);
		Image closeImage = Activator.getImageDescriptor("icons/close.png")
				.createImage();
		...
		final Composite recipientsComposite = new Composite(inputEntry,
				SWT.NONE);
		GridDataFactory.fillDefaults().grab(true, false).span(2, 1)
				.applyTo(recipientsComposite);
		...
		ProjectSpace projectSpace = WorkspaceManager.getProjectSpace(ModelUtil
				.getProject(modelElement));
		try {
			User user = OrgUnitHelper.getUser(projectSpace,
					modelElement.getCreator());
		...
		ImageHyperlink addButton = new ImageHyperlink(recipientsComposite,
				SWT.TOP);
		ImageHyperlink removeButton = new ImageHyperlink(recipientsComposite,
				SWT.TOP);
		ImageHyperlink acceptButton = new ImageHyperlink(recipientsComposite,
				SWT.TOP);
		Image addImage = Activator.getImageDescriptor("icons/add.png")
				.createImage();
		Image removeImage = Activator.getImageDescriptor("icons/remove.png")
				.createImage();
		Image acceptImage = Activator.getImageDescriptor("icons/accept.png")
				.createImage();
		...
		GridDataFactory.fillDefaults().align(SWT.END, SWT.CENTER)
				.grab(true, false).applyTo(acceptButton);
		inputText.setFocus();
			...
			public void mouseUp(MouseEvent e) {
				...
				WorkspaceManager
						...
						.getAllModelElementsbyClass(
								OrganizationPackage.eINSTANCE.getOrgUnit(),
								list);
